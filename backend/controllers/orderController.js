...
const getOrder=async(req,res,next)=>{
    try{
        //user comes from the middleware verifyIsLoggedIn
        const order=await Order.findById(req.params.id).populate("user","-password -isAdmin -__v -_id -createdAt -updatedAt").orFail();
        res.send(order)
    }catch(err){
        next(err)
    }
